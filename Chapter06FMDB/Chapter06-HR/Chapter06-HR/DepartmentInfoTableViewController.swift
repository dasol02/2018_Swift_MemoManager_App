...
class DepartmentInfoTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            case 2 :
                ...
                cell.detailTextLabel?.text = "\(self.departInfo.departAdd)"
            default :
                ()
            }
            ...
            state.tag = row.empCd
            state.addTarget(self, action: #selector(self.changeState(_:)), for: UIControl.Event.valueChanged)
    ...
    @objc func changeState(_ sender: UISegmentedControl){
        ...
        if self.empDAO.editState(empCd: empCd, stateCd: stateCd!) {
            let alert = UIAlertController(title: nil, message: "재직 상태가 변경되었습니다.", preferredStyle: UIAlertController.Style.alert)
